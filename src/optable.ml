...
let tlaops =
    [ Logic,
      List.map (fun (name, prec, fix, als, defn) -> (name, prec, fix, als, Some defn)) [
        "=>",            (1,  1 ),   Infix Non,   [],                   Implies ;
        "<=>",           (2,  2 ),   Infix Non,   [ "\\equiv" ],        Equiv ;
        "/\\",           (3,  3 ),   Infix Left,  [ "\\land" ],         Conj ;
        "\\/",           (3,  3 ),   Infix Left,  [ "\\lor" ],          Disj ;
        "\\neg",         (4,  4 ),   Prefix,      [ "~" ; "\\lnot" ],   Neg ;
        "=",             (5,  5 ),   Infix Non,   [],                   Eq ;
        "#",             (5,  5 ),   Infix Non,   [ "/=" ],             Neq ;
      ] ;
      Sets,
      List.map (fun (name, prec, fix, als, defn) -> (name, prec, fix, als, Some defn)) [
        "SUBSET",        (10, 13),   Prefix,      [],                   SUBSET ;
        "UNION",         (10, 13),   Prefix,      [],                   UNION ;
        "DOMAIN",        (10, 13),   Prefix,      [],                   DOMAIN ;
        "\\subseteq",    (5,  5 ),   Infix Non,   [],                   Subseteq ;
        "\\in",          (5,  5 ),   Infix Non,   [],                   Mem ;
        "\\notin",       (5,  5 ),   Infix Non,   [],                   Notmem ;
        "\\",            (8,  8 ),   Infix Non,   [],                   Setminus ;
        "\\intersect",   (8,  8 ),   Infix Left,  [ "\\cap" ],          Cap ;
        "\\union",       (8,  8 ),   Infix Left,  [ "\\cup" ],          Cup ;
      ] ;
      Sets,
      ...
      List.map (fun (name, prec, fix, als) -> (name, prec, fix, als, None)) [
        ...
        ":=",            (5,  5 ),   Infix Non,   [] ;
        "::=",           (5,  5 ),   Infix Non,   [] ;
        "\\oplus",       (10, 10),   Infix Left,  [ "(+)" ] ;
        "\\ominus",      (11, 11),   Infix Left,  [ "(-)" ] ;
        "\\odot",        (13, 13),   Infix Left,  [ "(.)" ] ;
        "\\oslash",      (13, 13),   Infix Non,   [ "(/)" ] ;
        "\\otimes",      (13, 13),   Infix Left,  [ "(\\X)" ] ;
...
let standard_form b =
  ...
  | Lteq          -> { (lookup "\\leq") with defn = Some Lteq }
  | Lt            -> { (lookup "<") with defn = Some Lt }
  | Gteq          -> { (lookup "\\geq") with defn = Some Gteq }
